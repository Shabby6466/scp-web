...
Deno.serve(async (req) => {
  console.log("send-teacher-invitation function called");
  ...
  try {
    ...
    const rateLimit = await checkPersistentRateLimit(
      supabase, identifier, 'send-teacher-invitation', 20, 60000
    );
    ...
    const allowedRoles = ["admin", "director", "school", "school_staff"];
    ...
    const { invitationId }: TeacherInvitationRequest = await req.json();
    console.log("Processing invitation:", invitationId);

    // Fetch invitation to validate school_id access
    ...
    const inviteUrl = `${req.headers.get("origin") || "https://littleledger.lovable.app"}/accept-teacher-invite?token=${invitation.invitation_token}`;

    const emailResponse = await resend.emails.send({
      from: "LittleLedger <onboarding@resend.dev>",
      to: [invitation.email],
      subject: `You're invited to join ${schoolName} on LittleLedger`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p><strong>${schoolName}</strong> has invited you to join their team on LittleLedger as a staff member.</p>
              
              <div class="details">
                <p><strong>Name:</strong> ${invitation.first_name} ${invitation.last_name}</p>
                ${branchName ? `<p><strong>Location:</strong> ${branchName}</p>` : ""}
                ${invitation.classroom ? `<p><strong>Classroom:</strong> ${invitation.classroom}</p>` : ""}
              </div>
