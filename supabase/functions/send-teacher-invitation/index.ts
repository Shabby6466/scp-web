...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { invitationId }: TeacherInvitationRequest = await reqon();
    ...
    const inviteUrl = `${req.headers.get("origin") || "https://SCP.lovable.app"}/accept-teacher-invite?token=${invitation.invitation_token}`;

    const emailResponse = await resend.emails.send({
      from: "SCP <onboarding@resend.dev>",
      to: [invitation.email],
      subject: `You're invited to join ${schoolName} on SCP`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p><strong>${schoolName}</strong> has invited you to join their team on SCP as a staff member.</p>
